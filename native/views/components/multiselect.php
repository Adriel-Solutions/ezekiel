...
>
    ...
    <div class="mt-1 relative">
        <button
            type="button"
            class="cursor-pointer bg-white relative w-full border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 sm:text-sm disabled:bg-gray-100"
        ...
        >
            <?php if ( isset($params['show_global_picker']) && $params['show_global_picker'] === true ): ?>
                <!-- PICK-ALL PICKER -->
                <li
                    class="select-none relative py-2 pl-3 pr-9 cursor-pointer"
                    x-data="{ highlighted: false }"
                    x-bind:class="{ 'text-white bg-blue-600': highlighted, 'text-gray-900': !highlighted }"
            ...
            <?php foreach($params['options'] as $value => $label): ?>
                ...
                >
                    ...
                    <span
                        ...
                        x-bind:class="{ 'text-white': highlighted, 'text-blue-600': !highlighted }"
                        x-show="selected"
                    >
    ...
    <?php if (!empty($params['values'])): ?>
        <input 
            type="hidden" 
            value="[<?= _e(join(',', array_map(function($v) { return "\"$v\""; }, $params['values']))) ?>]" 
            name="<?= $params['name'] ?>"
            x-init="setTimeout(() => { payload['<?= $params['name'] ?>'] = JSON.parse($el.value); }, 50)"
        />
    <?php endif; ?>
</div>
